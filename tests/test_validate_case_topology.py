...
def write_legacy_marker(path, allowlist=True, **overrides):
    if allowlist:
...
def test_legacy_case_requires_reason(tmp_path):
    ...
    write_legacy_marker(case_dir, reason="")
...
def test_legacy_case_rejects_far_future_expiry(tmp_path):
    case_dir = tmp_path / "cases" / "allowed-case"
    case_dir.mkdir(parents=True)
    write_legacy_marker(case_dir, expires_on="2026-07-15")
...
def test_legacy_case_rejects_expired_marker(tmp_path):
    case_dir = tmp_path / "cases" / "allowed-case"
    case_dir.mkdir(parents=True)
    write_legacy_marker(case_dir, expires_on="2026-05-14")
...
def test_legacy_case_is_visible_in_cli_output(tmp_path, capsys):
    ...
    assert "temporarily exempt until 2026-07-14" in captured.out
