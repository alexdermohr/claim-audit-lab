...
When `origin_source_refs` is explicitly declared, each id must resolve in `sources.yml`. Explicit origins may add source context but must not omit origins derived from the relevant evidence.
